...
impl Ratelimiter {
    ...
    #[instrument]
    pub async fn perform(&self, req: RatelimitedRequest<'_>, user_agent: Option<&String>) -> Result<Response> {
        ...
        loop {
            ...
            let request = req.build(&self.client, &self.token, None, user_agent)?.build()?;
